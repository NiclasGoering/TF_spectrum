...
MAX_PARALLEL_HUGE = 4    # For n_train >= 1000000
...
BATCH_SIZE_LARGE = 65536 # Batch size for large experiments (131072/2)
BATCH_SIZE_HUGE = 65536 # Batch size for huge experiments (131072*2)
...
def load_model_from_path(model_path, device):
    ...
    try:
        ...
        if isinstance(model_data, dict) and 'model_state_dict' in model_data:
            ...
            alignment = False  # Default alignment
        ...
        if input_dim is None or hidden_size is None or depth is None:
            raise ValueError(f"Could not determine model parameters (input_dim={input_dim}, hidden_size={hidden_size}, depth={depth}) from {model_path}")
        ...
        global_stats["error_types"][error_type] = global_stats["error_types"].get(error_type, 0) + 1
        global_stats["failed_models"].add(os.path.basename(model_path))
...
def generate_dataset_from_model(model, input_dim, n_samples, device):
    """Generate a dataset by passing random inputs through the model."""
    with torch.no_grad():
        # Generate random inputs
        X = torch.randn(n_samples, input_dim, device=device)
        
        # Run inputs through model to get outputs
        y = model(X)
...
def worker_process(gpu_id, num_gpus, all_combinations, config, full_results_dir, timestamp, checkpoint_log_path, completed_configs):
    ...
    try:
        ...
        for model_idx, (model_path, model_combos) in enumerate(by_model.items()):
            ...
            # Load model
            model, input_dim = load_model_from_path(model_path, device)
            ...
            try:
                # We'll generate the largest dataset needed for this model
                max_n_samples = max([c['n_train'] for c in model_combos]) + config["base_config"]['n_test']
                
                # Generate dataset on the fly
                print(f"[GPU {gpu_id}] Generating dataset with {max_n_samples} samples")
                data = generate_dataset_from_model(model, input_dim, max_n_samples, device)
                ...
                huge_exps = [c for c in model_combos if c['n_train'] >= 1000000]  # New category for huge experiments
                ...
                if large_exps:
                    for i in range(0, len(large_exps), MAX_PARALLEL_LARGE):
                        ...
                        try:
                            n_completed = fast_parallel_training(
                                batch, device, X_full, y_full, config["base_config"],
                                BATCH_SIZE_LARGE, 40, config["base_config"]["epochs"],  # Use full epochs
                                full_results_dir, timestamp, gpu_id, checkpoint_log_path, completed_configs
                            )
                            completed_count += n_completed
                            model_stats[model_basename]["completed"] += n_completed
                ...
                if huge_exps:
                    for i in range(0, len(huge_exps), MAX_PARALLEL_HUGE):
                        ...
                        try:
                            n_completed = fast_parallel_training(
                                batch, device, X_full, y_full, config["base_config"],
                                BATCH_SIZE_HUGE, 50, config["base_config"]["epochs"],  # Use full epochs
                                full_results_dir, timestamp, gpu_id, checkpoint_log_path, completed_configs
                            )
                            completed_count += n_completed
                            model_stats[model_basename]["completed"] += n_completed
...
def fast_parallel_training(config_batch, device, X_full, y_full, base_config, 
    ...
    Now with improved error handling and safe checkpoint updates.
    """
    # Get test data
    n_test = base_config['n_test']
    # Use model_path instead of ds_path
    fixed_seed = abs(hash(config_batch[0]['model_path'])) % (2**32)
    ...
    for config_item in config_batch:
        ...
        try:
            ...
            # Changed ds_name to model_name here
            sample_seed = hash(f"sample_{n_train}_{config_item['model_name']}_{config_item['experiment_num']}")
    ...
    for i in range(len(models)):
        try:
            ...
            result = {
                'model_name': config_item['model_name'],  # Changed from dataset_name
                'model_path': config_item['model_path'],  # Changed from dataset_path
...
def fast_training(config_item, device, X_full, y_full, base_config, 
    ...
    # Get test data
    n_test = base_config['n_test']
    # Changed from ds_path to model_path
    fixed_seed = abs(hash(config_item['model_path'])) % (2**32)
    generator = torch.Generator(device=device)
    generator.manual_seed(fixed_seed)
    indices = torch.randperm(len(X_full), device=device, generator=generator)
    test_indices = indices[:n_test]
    ...
    # Sample training data
    n_train = config_item['n_train']
    # Changed from ds_name to model_name
    sample_seed = hash(f"sample_{n_train}_{config_item['model_name']}_{config_item['experiment_num']}")
    torch.manual_seed(sample_seed)
    ...
    result = {
        ...
        'error_history': {
            ...
            'stopped_epoch': epoch if early_stopped else max_epochs + fine_tuning_epochs - 1
        },
        'worker_gpu': gpu_id,
    ...
    try:
        ...
        # Mark as completed with safe checkpoint update
        return safe_update_checkpoint(checkpoint_log_path, unique_id)
        
    except Exception as e:
        print(f"[GPU {gpu_id}] Error saving result for {unique_id}: {str(e)}")
        traceback.print_exc()
        ...
        with open(error_log_path, "a") as err_f:
            fcntl.flock(err_f, fcntl.LOCK_EX)
            err_f.write(f"{datetime.now()}: [GPU {gpu_id}] Error saving result for {unique_id}: {str(e)}\n")
            err_f.write(traceback.format_exc() + "\n")
            fcntl.flock(err_f, fcntl.LOCK_UN)
