...
MAX_PARALLEL_MEDIUM = 8  # For 10000 <= n_train < 100000
...
BATCH_SIZE_MEDIUM = 32768 # Batch size for medium experiments
BATCH_SIZE_LARGE = 131072 # Batch size for large experiments
...
def load_model_test_set(dataset_dir, device):
    """
    Load the model-specific test set from the dataset directory.
    
    Args:
        dataset_dir: Directory containing the model datasets and test set
        device: Torch device to load the data to
    
    Returns:
        X_test, y_test: Test data tensors or None if not found
    """
    try:
        # Look for test info file first
        test_info_files = glob.glob(os.path.join(dataset_dir, "test_info_*.json"))
        if test_info_files:
            # Load test information
            with open(test_info_files[0], 'r') as f:
                test_info = json.load(f)
            
            # Find test dataset file
            test_dataset_files = glob.glob(os.path.join(dataset_dir, "test_dataset_*.pt.gz"))
            if test_dataset_files:
                test_dataset_path = test_dataset_files[0]
                print(f"Found model-specific test set at {test_dataset_path}")
                
                # Load test dataset
                if test_dataset_path.endswith('.pt.gz'):
                    import gzip
                ...
                if isinstance(test_data, dict) and 'X_test' in test_data and 'y_test' in test_data:
                    X_test = test_data['X_test'].to(device, non_blocking=True)
                    y_test = test_data['y_test'].to(device, non_blocking=True)
                    return X_test, y_test
                else:
                    print(f"Unexpected test data format in {test_dataset_path}")
            else:
                print(f"No test dataset file found in {dataset_dir}")
        else:
            print(f"No test info file found in {dataset_dir}")
        
        return None, None
        
    except Exception as e:
        print(f"Error loading model test set: {str(e)}")
        traceback.print_exc()
        return None, None
...
def load_dataset_info(directory):
    ...
    # Look for compressed .pt.gz files first
    dataset_files = glob.glob(os.path.join(directory, "dataset_*.pt.gz"))
    if not dataset_files:
        # Fall back to regular .pt files if no .pt.gz files found
    ...
    if dist_type and input_dim and hidden_size != "":
        ...
        ds_name = os.path.basename(dataset_path).replace("dataset_", "").replace(".pt.gz", "").replace(".pt", "")
    ...
    try:
        ...
        for ds_path, dataset_combos in by_dataset.items():
            ...
            try:
                ...
                # First try to load model-specific test set
                dataset_dir = os.path.dirname(ds_path)
                X_test, y_test = load_model_test_set(dataset_dir, device)
                
                # If model-specific test set not found, fall back to traditional approach
                if X_test is None or y_test is None:
                    print(f"[GPU {gpu_id}] No model-specific test set found, falling back to dataset split")
                    n_test = config["base_config"]['n_test']
                    fixed_seed = abs(hash(dataset_combos[0]['ds_path'])) % (2**32)
                    ...
                    # If we're using model-specific test set, all data is training data
                    train_master_indices = torch.arange(len(X_full), device=device)
                    print(f"[GPU {gpu_id}] Using model-specific test set with shape: {X_test.shape}, {y_test.shape}")
                ...
                if tiny_exps:
                    for i in range(0, len(tiny_exps), MAX_PARALLEL_TINY):
                        ...
                        n_completed = fast_parallel_training(
                            ...
                            X_test=X_test, y_test=y_test, train_master_indices=train_master_indices
                        )
                    ...
                    for exp in large_exps:
                        ...
                        try:
                            ...
                            fast_training(
                                ...
                                BATCH_SIZE_LARGE, config["base_config"]["epochs"],
            ...
            del X_full, y_full, data, X_test, y_test
            torch.cuda.empty_cache()
...
def fast_parallel_training(config_batch, device, X_full, y_full, base_config, 
    ...
    if X_test is None or y_test is None:
        ...
        X_test = X_full[test_indices]
        y_test = y_full[test_indices]
    elif train_master_indices is None:
        # If test set is provided but not train indices, use all indices
        train_master_indices = torch.arange(len(X_full), device=device)
