...
def save_test_dataset_compressed(X_test, y_test, filepath, rank):
    """
    Save test dataset with compression.
    
    Args:
        X_test: Test input tensor
        y_test: Test output tensor
        filepath: Path to save the compressed test dataset
        rank: MPI rank (for logging)
    """
    print(f"Process {rank}: Compressing test dataset...")
    
    # Convert to float32 to save space
    X_f32 = X_test.detach().cpu().to(torch.float32)
    y_f32 = y_test.detach().cpu().to(torch.float32)
    
    # Create a buffer to compress the data
    buffer = io.BytesIO()
    torch.save({'X_test': X_f32, 'y_test': y_f32}, buffer)
    ...
    print(f"Process {rank}: Saved compressed test dataset ({file_size_mb:.2f} MB) to {filepath}")
...
def main():
    ...
    num_experiments = 1  # Number of experiments to run with different random seeds
    
    # Hidden layer sizes to explore
    hidden_sizes = [ 2, 4, 8, 16] # 32,64,128    2, 4, 8, 16
    ...
    train_size = 5000000       # Number of training samples.
    ...
    for dist_type, d, alpha, r_value, hidden_size, exp_num in my_combinations:
        ...
        save_dir = os.path.join("/home/goring/TF_spectrum/pretrain/grid_1803_globtest2/",
        ...
        # === Generate a separate test set using a different seed ===
        # Use a completely different seed for the test set to ensure independence
        test_seed = train_seed + 10000  # Ensure test seed is very different from train seed
        torch.manual_seed(test_seed)
        np.random.seed(test_seed)
        
        # Generate test data with the same distribution parameters but different seed
        X_test = generate_data(dist_type, n_test, d, device, r=r_value if r_value is not None else 0.5)
        ...
        test_dataset_path = os.path.join(save_dir, f"test_dataset_{smart_name}.pt.gz")
        save_test_dataset_compressed(X_test, y_test, test_dataset_path, rank)
        ...
        results = {
            'hyperparameters': {
                ...
                'test_seed': test_seed,  # Add test seed information
                'test_size': n_test  # Add test size information
            },
        ...
        # Store information about the test set
        test_set_info = {
            'train_seed': train_seed,
            'test_seed': test_seed,
            'test_size': n_test,
        ...
        # Save test set information with the dataset
        test_info_path = os.path.join(save_dir, f"test_info_{smart_name}.json")
