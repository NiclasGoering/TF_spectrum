...
def main():
    ...
    dimensions = [32,64,128]   #32,64,128 #4,8,16
    
    # Alpha values to explore for the selected spectrum type
    alpha_values = [0.0,0.5,1.0]   #0.0,0.5,1.0
    ...
    hidden_sizes = [2, 4,8,16,32,64,128] # 8 # Multiple hidden sizes to explore # 4,8,16,32,64
    ...
    train_size = 10000000         # Number of training samples.
    ...
    for dist_type, d, alpha, r_value, hidden_size, exp_num in my_combinations:
        # Generate a deterministic seed based on parameters
        data_generation_seed = hash(f"{dist_type}_{d}_{alpha}_{r_value}_{hidden_size}_{exp_num}") % (2**32)
        ...
        save_dir = os.path.join("/home/goring/TF_spectrum/pretrain/paper_grid_1703/",
        ...
        save_info = {
            ...
            'data_generation_seed': data_generation_seed,  # Save seed
            'distribution_type': dist_type,                # Save distribution type
            'distribution_params': {                       # Save any distribution parameters
                'r_value': r_value
            }
        }
